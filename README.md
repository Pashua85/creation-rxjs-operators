...
<a href="https://stackblitz.com/edit/rxjs-rghhyk">![Of example scneenshot](/assets/of-screen.png)</a>
...
Оператор `from` создает `Observable` только на основе итерируемого значения или промиса. В случае промиса в поток попадет результат его выполнения, а при работе с итерируемым значением элементы этого значения попадают в поток по очереди.
